...
export default function WeekdayHourHeatmap({ heatmap, isPartial }: { heatmap: WeekdayHourHeatmapResult; isPartial: boolean }) {
  return <section className="panel heatmap-panel" aria-labelledby="heatmap-heading">
    ...
    <div className="heatmap-scroller"><table className="heatmap-table" aria-label="曜日と時間帯ごとの自分の発言数"><thead><tr><th scope="col" className="sticky-cell">曜日</th>{Array.from({ length: 24 }, (_, hour) => <th key={hour} scope="col">{hour}</th>)}</tr></thead><tbody>{weekdayLabels.map((label, weekday) => <tr key={label}><th scope="row" className="sticky-cell">{label}</th>{heatmap.cells.slice(weekday * 24, weekday * 24 + 24).map((cell) => <td key={cell.hour}><span tabIndex={0} aria-label={`${label}曜日 ${cell.hour}時台 ${cell.count}件`} className={`heat-cell ${intensityClass(cell.count, heatmap.maxCount)}`}>{cell.count === 0 ? "·" : cell.count}</span></td>)}</tr>)}</tbody></table></div>
    ...
    <p className="heatmap-note">スマートフォンでは表を左右にスクロールして確認できます。各セルの件数はキーボードでも確認できます。</p>
  </section>;
}
